...
- NVIDIA ASR uses the hosted audio-capable chat completions path and normalizes the returned transcript into one full-length `TranscriptSegment`. This keeps the desktop workflow usable, but it is not a fine-grained timestamped ASR path.
